...
function saleTime(text){
 // No year guessing, no use of performance date, no account DOM.
 const date='(20\\d{2}[-/.年]\\d{1,2}[-/.月]\\d{1,2}日?[ T\\s]+\\d{1,2}:\\d{2}(?::\\d{2})?)';
 const a=String(text).match(new RegExp('(?:开售时间|开票时间|开售|开票)[：:\\s]*'+date));
 const b=String(text).match(new RegExp(date+'[\\s]*(?:开售|开票)'));
 return parseChinaTime((a||b||[])[1]);
}
function cleanTitle(s){return String(s||'未命名演出').replace(/\s+/g,' ').trim().slice(0,160);}
function normalize(raw,now=Date.now()){
 const u=itemURL(raw.url);if(!u)return null;
 const title=cleanTitle(raw.title),opensAt=saleTime(String(raw.saleText||'').slice(0,120));
...
const api={itemURL,parseChinaTime,saleTime,normalize,merge,status,money,plan};root.TicketCatalog=api;if(typeof module!=='undefined')module.exports=api;
})(globalThis);
